...
class ChunkHandlerServiceTest {
    ...
    fun `each sentence becomes its own chunk`() {
        ...
        val chunks = buildChunks(sentences, "msg-1", counter)
    ...
    fun `chunk sequence numbers start at 0 and increment`() {
        ...
        val chunks = buildChunks(sentences, "msg-2", AtomicLong(1))
    ...
    fun `faissIds increment sequentially from the given counter`() {
        ...
        val chunks = buildChunks(sentences, "msg-3", AtomicLong(500))
    ...
    @Test
    fun `messageId is set on every chunk`() {
        val sentences = listOf("Sentence A.", "Sentence B.")
        val chunks = buildChunks(sentences, "conv-99", AtomicLong(1))

        chunks.forEach { assertEquals("conv-99", it.messageId) }
    }

    @Test
    fun `blank sentences are filtered out`() {
        val sentences = listOf("Real content.", "", "   ", "Also real.", "")
        val chunks = buildChunks(sentences, "msg-4", AtomicLong(1))
    ...
    fun `sentences exceeding maxChars are truncated`() {
        ...
        val chunks = buildChunks(listOf(sentence), "msg-5", AtomicLong(1), maxChars = 50)
    ...
    fun `empty input produces empty list`() {
        val chunks = buildChunks(emptyList(), "msg-6", AtomicLong(1))
        assertTrue(chunks.isEmpty())
    }

    @Test
    fun `default counter starts from non-zero`() {
        val chunks = buildChunks(listOf("Test."), "msg-7")
    ...
    fun `paragraph with multiple related sentences produces one chunk per sentence`() {
        val paragraph = "The transformer architecture introduced the attention mechanism. " +
        ...
        val chunks = buildChunks(sentences, "msg-8", AtomicLong(1))
    ...
    fun `two related short sentences remain separate chunks`() {
        ...
        val chunks = buildChunks(sentences, "msg-9", AtomicLong(1))
    ...
    fun `non-related sentence does not merge with preceding context`() {
        ...
        val chunks = buildChunks(sentences, "msg-10", AtomicLong(1))

        assertEquals(3, chunks.size)
        assertEquals("The attention mechanism is the core innovation of transformers.", chunks[0].text)
        assertEquals("I also tried a new pasta recipe yesterday and it was delicious.", chunks[1].text)
        assertEquals("Multi-head attention allows the model to focus on different parts of the input.", chunks[2].text)
        assertEquals(0, chunks[0].sequence)
        assertEquals(1, chunks[1].sequence)
        assertEquals(2, chunks[2].sequence)
    }
    ...
    fun `realistic message with mixed topics produces correct chunks`() {
        val paragraph = "Hey, I was reading about transformers in deep learning. " +
        ...
        val chunks = buildChunks(sentences, "msg-11", AtomicLong(1))
